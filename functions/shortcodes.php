...
function site_identity_func( $atts = array(), $content = null ) {
	...
	$logo_url = ($naturalbeauty_options['logo']['url']) ? $naturalbeauty_options['logo']['url'] : get_template_directory_uri(). '/images/logo.png';
	$logo_option = $naturalbeauty_options['logo-option'];
	$html = '';
...
function email_func( $atts = array(), $content = '' ) {	
	global $naturalbeauty_options;
	$contact_email = $naturalbeauty_options['contact-email'];
	$html = '';	
	$atts = shortcode_atts( array(
		'offset' => 0,
		'index' => 0,
...
function phone_func( $atts = array(), $content = '' ) {
	...
	if ($atts['index']) :
	    ...
	    $html .= preg_replace('/[^0-9]/', '', $naturalbeauty_options['contact-phone'][$i]);
	    $html .= '" >';
	    $html .= $naturalbeauty_options['contact-phone'][$i];  
	    $html .= '</a>';
	    $html .= '</span>';		
	else :
		foreach ($naturalbeauty_options['contact-phone'] as $phone) :
			if ($n > $atts['offset']) :
			    $html .= '<span class="phone-number">';
			    $html .= '<a class="phoneToShow" href="tel:';
...
function fax_func( $atts = array(), $content = '' ) {
	...
	if ($atts['index']) :
	    ...
	    $html .= preg_replace('/[^0-9]/', '', $naturalbeauty_options['contact-fax'][$i]);
	    $html .= '" >';
	    $html .= $naturalbeauty_options['contact-fax'][$i];  
	    $html .= '</a>';
	    $html .= '</span>';		
	else :
		foreach ($naturalbeauty_options['contact-fax'] as $fax) :
			if ($n > $atts['offset']) :
			    $html .= '<span class="fax-number">';
			    $html .= '<a class="faxToShow" href="tel:';
...
function address_func( $atts = array(), $content = '' ) {
	global $naturalbeauty_options;
	$contact_address = $naturalbeauty_options['contact-address'];
	$html = '';
	$atts = shortcode_atts( array(
		'offset' => 0,
		'map' => 0,
...
function social_menu_fnc( $atts = array(), $content = '' ) {
	...
	global $naturalbeauty_options;
	$html = '';
	$contact_social = $naturalbeauty_options['contact-social'];
	$contact_address = $naturalbeauty_options['contact-address'];
	$atts = shortcode_atts( array(
		'display' => 'inline',
		'title' => 0,
...
function link_func( $atts = array(), $content = '' ) {
	$atts = shortcode_atts( array(
		'to' => 1,
		'title' => '',
		'class' => '',
	...
	$html = '';
	if (is_numeric ($atts['to'])) {
		$id = $atts['to'];
	} elseif ($atts['to'] == 'latest-blog' OR $atts['to'] == 'latest-post') {
		$latest_post = get_posts("post_type=post&numberposts=1");
		$id = $latest_post[0]->ID;		
	} elseif (!preg_match('/\s/',$atts['to'])) {
		$page = get_page_by_path( $atts['to'] );
		$id = $page->ID;
	}
	if ($atts['title']) $title = $atts['title'];
	else $title = get_the_title( $id );
	return $html = '<a class="'.$atts['class'].'" href="'.get_the_permalink( $id ).'">'.$title.'</a>';
}
