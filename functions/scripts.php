...
function naturalbeauty_enqueue_scripts() {
	global $naturalbeauty_options;
	wp_enqueue_script( 'jquery' );	
	wp_register_style( 'google-font', 'https://fonts.googleapis.com/css?family=Cormorant+Garamond:300,400,500,600,700|Lora:400,700|Montserrat:100,200,300,400,500,600,700,800,900' );
	...
	if ($naturalbeauty_options['basic-styling-stylesheet']) {
		wp_register_style( $naturalbeauty_options['basic-styling-stylesheet'], get_template_directory_uri() .  '/css/'.$naturalbeauty_options['basic-styling-stylesheet'].'.css', array('main.min'));
		wp_enqueue_style( $naturalbeauty_options['basic-styling-stylesheet'] );		
	}

}
add_action( 'wp_enqueue_scripts', 'naturalbeauty_enqueue_scripts' );
function naturalbeauty_admin_enqueue_scripts(){
	wp_register_style( 'font-awesome.min', get_template_directory_uri() . '/fonts/font-awesome-4.7.0/css/font-awesome.min.css' );
...
add_action( 'admin_enqueue_scripts', 'naturalbeauty_admin_enqueue_scripts' );
